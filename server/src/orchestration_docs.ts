...
function buildOrchestratorPolicy(input: OrchestrationDocsInput): string {
  ...
  return [
    ...
    "- Demand reproducible evidence packs before accepting claims.",
    "- Keep risky actions behind explicit review and approval gates.",
    "",
    ...
    "",
    "## Risk-tier decision protocol",
    "- LOW risk: doc updates, scoped refactors, deterministic tests -> orchestrator may auto-approve with evidence.",
    "- MEDIUM risk: behavior changes in owned scope -> require fresh repro + verification evidence before approve.",
    "- HIGH risk: scope expansion, destructive ops, security/auth changes, production-impacting config -> require hard review and explicit user confirmation.",
    "- If tier is unclear, treat as HIGH until clarified.",
    "",
    "## Reproducibility contract (benchmark-aligned)",
    "- Every significant fix attempt must include a repro pack:",
    "- REPRO PACK fields: base commit, exact command, expected fail/pass signal, runtime/tool version, timestamp.",
    "- Use identical verify commands before/after fix where possible.",
    "- If behavior is non-deterministic across reruns, pause rollout and isolate cause.",
    "",
    "## Security posture contract",
    "- For security-relevant changes, run SAST/secret/dependency checks before integration.",
    "- Record scan command + result summary in worker evidence section.",
    "- Missing security evidence blocks approval for high-risk surfaces.",
    "",
...
function buildCommandBusDoc(input: OrchestrationDocsInput): string {
  ...
  return [
    ...
    "```",
    "",
    "Structured directive variants (also accepted):",
    "```text",
    "FYP_SEND_TASK_JSON: {\"target\":\"worker:<name>\",\"text\":\"TASK...\"}",
    "FYP_ANSWER_QUESTION_JSON: {\"attentionId\":\"<id>\",\"optionId\":\"approve\",\"responseText\":\"<notes>\"}",
    "FYP_QUESTION_RESPONSE_JSON: {\"attentionId\":\"<id>\",\"optionId\":\"approve\"}",
    "```",
    "",
...
function buildWorkerTaskDoc(input: OrchestrationDocsInput, worker: OrchestrationDocsWorker): string {
  ...
  return [
    ...
    "- If risk is MEDIUM/HIGH, include explicit risk tier and expected blast radius in status.",
    "- Completion claims without fresh command output are invalid.",
    "",
    "## Checklist",
    "- [ ] Read current orchestrator dispatch",
    "- [ ] Confirm owned scope and non-goals",
    "- [ ] Record planned files before editing",
    "- [ ] Build repro pack (base commit + exact commands + expected signals)",
    "- [ ] Classify risk tier (LOW/MEDIUM/HIGH) before implementation",
    "- [ ] Implement scoped changes only",
    "- [ ] Run verification command(s)",
    "- [ ] Run security checks when auth/security/supply-chain surface changed",
    ...
    "- Observed pre-fix signal:",
    "- Verify command:",
    "- Observed post-fix signal:",
